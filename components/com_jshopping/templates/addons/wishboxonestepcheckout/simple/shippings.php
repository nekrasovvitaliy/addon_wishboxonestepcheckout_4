  ...
  <tr>
    <td style = "padding-top:5px; padding-bottom:5px">
      <label for = "shipping_method_<?php echo $shipping->sh_pr_method_id ?>">
					<input type="radio" name="sh_pr_method_id" id="shipping_method_<?php echo $shipping->sh_pr_method_id ?>" value="<?php echo $shipping->sh_pr_method_id ?>" <?php if ($shipping->sh_pr_method_id==$this->active_shipping){ ?>checked="checked"<?php } ?> onclick="oneStepCheckout.showShippingForm('<?php echo $shipping->shipping_id ?>', 1)" />
		...
		<tr id="tr_shipping_<?php echo $shipping->shipping_id ?>" <?php if ($this->addonParams->shipping_params && $shipping->sh_pr_method_id!=$this->active_shipping){ ?>style="display:none"<?php } ?>>
