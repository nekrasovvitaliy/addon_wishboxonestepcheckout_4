...
<div class="uk-panel uk-panel-box uk-panel-header finish-page">
	...
	<dl class="uk-description-list uk-description-list-horizontal"> 
		...
		<dd><?php echo \JSHelper::formatprice($this->order->order_subtotal, $this->order->currency_code) ?></dd>
		<?php } ?>
		...
		<dd><?php echo \JSHelper::formatprice(-$this->order->order_discount, $this->order->currency_code) ?></dd>
		<?php } ?>
		...
		<dd><?php echo \JSHelper::formatprice($this->order->order_total - $this->order_total, $this->order->currency_code) ?></dd>
		<?php } ?>
		<?php } ?>
		<?php if ($this->addonParams->order_payment && $this->order->payment_method_id > 0) { ?>
		<dt><?php echo $this->order->payment_name ?></dt>
		<dd><?php echo \JSHelper::formatprice($this->order->order_payment, $this->order->currency_code) ?></dd>
		<?php } ?>
		<?php if ($this->addonParams->order_shipping && $this->order->shipping_method_id > 0) { ?>
		<dt><?php echo $this->order->shipping_name ?></dt>
		<dd><?php echo \JSHelper::formatprice($this->order->order_shipping, $this->order->currency_code) ?></dd>
		<?php } ?>
		...
		<dd><?php echo \JSHelper::formatprice($this->order->order_package, $this->order->currency_code) ?></dd>
		<?php } ?>
		<?php if ($this->addonParams->order_total) { ?>
		<dt><?php echo Text::_('JSHOP_ONESTEPCHECKOUT_FINISH_ORDER_TOTAL') ?></dt>
		<dd><?php echo \JSHelper::formatprice($this->order->order_total, $this->order->currency_code) ?></dd>
		<?php } ?>
		<?php
		...
		<dd>
			<table class="minicart uk-table">
				...
				<tr class="jshop_prod_cart">
					<td>
						<a href="<?php echo \JSHelper::SEFLink('index.php?option=com_jshopping&controller=product&task=view&category_id='.$product->category_id.'&product_id='.$product->product_id, 1)?>">
							<?php
						...
						<br />
						<?php echo \JSHelper::sprintAtributeInOrder($item->product_attributes).sprintFreeAtributeInOrder($item->product_freeattributes) ?>
					</td>
					<td>
						<?php echo \JSHelper::formatprice($item->product_item_price, $this->order->currency_code) ?>
					</td>
					<td>
						<?php echo \JSHelper::formatqty($item->product_quantity) ?>
					</td>
					<td>
						<?php echo \JSHelper::formatprice($item->product_quantity * $item->product_item_price, $this->order->currency_code) ?>
					</td>
				</tr>
				<?php } ?>
				</tbody>
