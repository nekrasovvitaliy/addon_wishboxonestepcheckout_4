...
<div class="jshop">
	...
	<table id = "table_shippings" cellspacing="0" cellpadding="0">
		...
		<tr>
			<td style = "padding-top:5px; padding-bottom:5px">
				<label for = "shipping_method_<?php echo $shipping->sh_pr_method_id; ?>">
					<input
						type="radio"
						name="sh_pr_method_id"
						id="shipping_method_<?php echo $shipping->sh_pr_method_id; ?>"
						value="<?php echo $shipping->sh_pr_method_id; ?>"
						<?php if ($shipping->sh_pr_method_id == $this->active_shipping) { ?>
						checked="checked"
						<?php } ?>
						onclick="oneStepCheckout.showShippingForm('<?php echo $shipping->sh_pr_method_id ?>', 1)"
					/>
		...
		<tr
			id="tr_shipping_<?php echo $shipping->sh_pr_method_id ?>"
			<?php if ($this->addonParams->shipping_params && $shipping->sh_pr_method_id!=$this->active_shipping){ ?>style="display:none"<?php } ?>
		>
					...
					<tr>
						<td class="weight">
								<?php
									...
									{
										echo \JSHelper::formatweight($price->shipping_weight_from).' - '.\JSHelper::formatweight($price->shipping_weight_to);
									}
									else
									{
										echo _JSHOP_FROM.' '.\JSHelper::formatweight($price->shipping_weight_from);
									}
								?>
						</td>
						<td class="price">
...
<div class="jshop">
	...
	<label for="package">
		<?php echo Text::_('JSHOP_ONESTEPCHECKOUT_PACKAGE_LABEL'); ?>
	</label>
	<?php if ($this->addonParams->package_image != '-1') { ?>
	<div id="package_image">
		<img src="<?php echo URI::root(true).'/components/com_jshopping/templates/addons/wishboxonestepcheckout/'.$this->addonParams->template.'/images/'.$this->addonParams->package_image; ?>" />
	</div>
	<?php } ?>
	<?php if ($this->addonParams->package_text) { ?>
	<div id="package_text">
		<?php echo Text::_('JSHOP_ONESTEPCHECKOUT_PACKAGE_TEXT'); ?>
	</div>
	<?php } ?>
</div>
<?php } ?>